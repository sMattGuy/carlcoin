...
function testResponses(client,message){
	...
	message.channel.send('ACTIONS:\n!cc attack\n!cc look\n!cc block\n!cc run\n!cc item').then(()=>{message.channel.awaiteMessages(filter,{max:1,time:20000,errors:['time']}).then(choice => {message.channel.send(`You selected ${choice}`)})}).catch(e => {message.channel.send(`Didnt get valid response in time`);console.log(e);});
}
